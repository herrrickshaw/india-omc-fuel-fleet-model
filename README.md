...
| `build_energy_doc.js` → `docs/Energy_Blend_Volume_Dividend.docx/.pdf` | 10-section paper with methodology/assumptions/caveats per section and a 16-row abbreviations glossary (docx-js; do not hand-edit outputs). |
| `build_slides.js` → `docs/Volume_Dividend_Slides.pptx` | 12-slide deck: mechanism → who collects → blend walk → consumer parity prices → funding scenarios → **ethanol SGST (E27@5% ≈ ₹4,787 cr/yr, price-neutral)** → the E27 grand bargain (parity ₹98.44 + SGST still leaves ₹1.27/L slack) → CBG → supply check → RON95. |
